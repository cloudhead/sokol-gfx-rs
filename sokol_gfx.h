...
/* rendering functions */
SOKOL_API_DECL void
                    sg_begin_default_pass(const sg_pass_action * pass_action, int width, int height);
...
#ifdef SOKOL_IMPL
#if !(                                                                                             \
    defined(SOKOL_GLCORE33) || defined(SOKOL_GLES2) || defined(SOKOL_GLES3) ||                     \
...
#define SOKOL_GLES2_INSTANCING_ERROR                                                               \
	"Select GL_ANGLE_instanced_arrays or (GL_EXT_draw_instanced & GL_EXT_instanced_arrays) "   \
	"to enable instancing in GLES2"
#define glDrawArraysInstanced(mode, first, count, instancecount)                                   \
...
typedef struct {
	...
	bool            ext_textures; /* if true, external textures were injected with
	                                 sg_image_desc.gl_textures */
} _sg_image_t;
...
enum { _SG_MTL_DEFAULT_UB_SIZE                = 4 * 1024 * 1024,
...
_SOKOL_PRIVATE int _sg_vertexformat_bytesize(sg_vertex_format fmt)
{
	...
	default: SOKOL_UNREACHABLE; return -1;
	}
}
...
_SOKOL_PRIVATE bool _sg_is_compressed_pixel_format(sg_pixel_format fmt)
{
	...
	case SG_PIXELFORMAT_ETC2_SRGB8: return true;
...
_SOKOL_PRIVATE bool _sg_is_valid_rendertarget_color_format(sg_pixel_format fmt)
{
	...
	case SG_PIXELFORMAT_RGBA16F: return true;
	default: return false;
	}
}
...
_SOKOL_PRIVATE bool _sg_is_valid_rendertarget_depth_format(sg_pixel_format fmt)
{
	...
	case SG_PIXELFORMAT_DEPTHSTENCIL: return true;
	default: return false;
	}
}
...
_SOKOL_PRIVATE int _sg_row_pitch(sg_pixel_format fmt, int width)
{
	...
	default: pitch = width * _sg_pixelformat_bytesize(fmt); break;
	}
	return pitch;
}
...
_SOKOL_PRIVATE int _sg_surface_pitch(sg_pixel_format fmt, int width, int height)
{
	...
	case SG_PIXELFORMAT_PVRTC2_RGBA:
	case SG_PIXELFORMAT_PVRTC4_RGB:
	case SG_PIXELFORMAT_PVRTC4_RGBA: num_rows = ((height + 3) / 4); break;
	default: num_rows = height; break;
	}
	if (num_rows < 1) {
		num_rows = 1;
	}
...
_SOKOL_PRIVATE void
_sg_resolve_default_pass_action(const sg_pass_action * from, sg_pass_action * to)
{
...
_SOKOL_PRIVATE sg_resource_state _sg_create_shader(_sg_shader_t * shd, const sg_shader_desc * desc)
{
	...
	for (int stage_index = 0; stage_index < SG_NUM_SHADER_STAGES; stage_index++) {
		...
		for (int ub_index = 0; ub_index < SG_MAX_SHADERSTAGE_UBS; ub_index++) {
			const sg_shader_uniform_block_desc * ub_desc =
...
_SOKOL_PRIVATE sg_resource_state
               _sg_create_pipeline(_sg_pipeline_t * pip, _sg_shader_t * shd, const sg_pipeline_desc * desc)
{
...
_SOKOL_PRIVATE sg_resource_state
               _sg_create_pass(_sg_pass_t * pass, _sg_image_t ** att_images, const sg_pass_desc * desc)
{
	...
	for (int i = 0; i < SG_MAX_COLOR_ATTACHMENTS; i++) {
		...
		if (att_desc->image.id != SG_INVALID_ID) {
			pass->num_color_atts++;
			SOKOL_ASSERT(
			    att_images[i] && (att_images[i]->slot.id == att_desc->image.id));
			SOKOL_ASSERT(
			    _sg_is_valid_rendertarget_color_format(att_images[i]->pixel_format));
...
_SOKOL_PRIVATE void _sg_apply_bindings(
    _sg_pipeline_t * pip, _sg_buffer_t ** vbs, const int * vb_offsets, int num_vbs,
    _sg_buffer_t * ib, int ib_offset, _sg_image_t ** vs_imgs, int num_vs_imgs,
...
_SOKOL_PRIVATE void
_sg_apply_uniforms(sg_shader_stage stage_index, int ub_index, const void * data, int num_bytes)
{
...
_SOKOL_PRIVATE void
_sg_append_buffer(_sg_buffer_t * buf, const void * data, int data_size, bool new_frame)
{
	SOKOL_ASSERT(buf && data && (data_size > 0));
	_SOKOL_UNUSED(data);
	_SOKOL_UNUSED(data_size);
...
_SOKOL_PRIVATE GLenum _sg_gl_vertexformat_type(sg_vertex_format fmt)
{
	...
	case SG_VERTEXFORMAT_FLOAT4: return GL_FLOAT;
	case SG_VERTEXFORMAT_BYTE4:
	case SG_VERTEXFORMAT_BYTE4N: return GL_BYTE;
	case SG_VERTEXFORMAT_UBYTE4:
	case SG_VERTEXFORMAT_UBYTE4N: return GL_UNSIGNED_BYTE;
	case SG_VERTEXFORMAT_SHORT2:
	case SG_VERTEXFORMAT_SHORT2N:
	case SG_VERTEXFORMAT_SHORT4:
	case SG_VERTEXFORMAT_SHORT4N: return GL_SHORT;
...
_SOKOL_PRIVATE GLboolean _sg_gl_vertexformat_normalized(sg_vertex_format fmt)
{
	...
	case SG_VERTEXFORMAT_UINT10_N2: return GL_TRUE;
...
_SOKOL_PRIVATE GLenum _sg_gl_teximage_type(sg_pixel_format fmt)
{
	switch (fmt) {
	case SG_PIXELFORMAT_RGBA32F:
	case SG_PIXELFORMAT_R32F: return GL_FLOAT;
	case SG_PIXELFORMAT_RGBA16F:
	case SG_PIXELFORMAT_R16F: return GL_HALF_FLOAT;
	case SG_PIXELFORMAT_RGBA8:
	case SG_PIXELFORMAT_RGB8:
	case SG_PIXELFORMAT_L8: return GL_UNSIGNED_BYTE;
	case SG_PIXELFORMAT_R10G10B10A2: return GL_UNSIGNED_INT_2_10_10_10_REV;
	case SG_PIXELFORMAT_R5G5B5A1: return GL_UNSIGNED_SHORT_5_5_5_1;
	case SG_PIXELFORMAT_R5G6B5: return GL_UNSIGNED_SHORT_5_6_5;
	case SG_PIXELFORMAT_RGBA4: return GL_UNSIGNED_SHORT_4_4_4_4;
	case SG_PIXELFORMAT_DEPTH:
		/* FIXME */
...
{
	switch (fmt) {
	case SG_PIXELFORMAT_NONE: return 0;
	case SG_PIXELFORMAT_RGBA8:
	case SG_PIXELFORMAT_R5G5B5A1:
	case SG_PIXELFORMAT_RGBA4:
	case SG_PIXELFORMAT_RGBA32F:
	case SG_PIXELFORMAT_RGBA16F:
	case SG_PIXELFORMAT_R10G10B10A2: return GL_RGBA;
	case SG_PIXELFORMAT_RGB8:
	case SG_PIXELFORMAT_R5G6B5: return GL_RGB;
...
#endif
	case SG_PIXELFORMAT_DEPTH: return GL_DEPTH_COMPONENT;
	case SG_PIXELFORMAT_DEPTHSTENCIL: return GL_DEPTH_STENCIL;
	case SG_PIXELFORMAT_DXT1: return GL_COMPRESSED_RGBA_S3TC_DXT1_EXT;
	case SG_PIXELFORMAT_DXT3: return GL_COMPRESSED_RGBA_S3TC_DXT3_EXT;
	case SG_PIXELFORMAT_DXT5: return GL_COMPRESSED_RGBA_S3TC_DXT5_EXT;
	case SG_PIXELFORMAT_PVRTC2_RGB: return GL_COMPRESSED_RGB_PVRTC_2BPPV1_IMG;
	case SG_PIXELFORMAT_PVRTC4_RGB: return GL_COMPRESSED_RGB_PVRTC_4BPPV1_IMG;
	case SG_PIXELFORMAT_PVRTC2_RGBA: return GL_COMPRESSED_RGBA_PVRTC_2BPPV1_IMG;
	case SG_PIXELFORMAT_PVRTC4_RGBA: return GL_COMPRESSED_RGBA_PVRTC_4BPPV1_IMG;
...
_SOKOL_PRIVATE GLenum _sg_gl_teximage_internal_format(sg_pixel_format fmt)
{
	...
	if (_sg.gl.gles2) {
		...
		case SG_PIXELFORMAT_RGB8: return GL_RGB8;
		case SG_PIXELFORMAT_RGBA4: return GL_RGBA4;
		case SG_PIXELFORMAT_R5G6B5:
			...
			return GL_RGB5;
#endif
		case SG_PIXELFORMAT_R5G5B5A1: return GL_RGB5_A1;
		case SG_PIXELFORMAT_R10G10B10A2: return GL_RGB10_A2;
		...
		case SG_PIXELFORMAT_L8: return GL_R8;
		case SG_PIXELFORMAT_DEPTH:
			/* FIXME */
...
/* clear initial GL error state */
	...
	_sg.gl.features[SG_FEATURE_TEXTURE_COMPRESSION_PVRTC] =
	    strstr(ext, "_texture_compression_pvrtc") || strstr(ext, "_compressed_texture_pvrtc");
	_sg.gl.features[SG_FEATURE_TEXTURE_COMPRESSION_ATC] =
	    strstr(ext, "_compressed_texture_atc");
	_sg.gl.ext_anisotropic = strstr(ext, "_texture_filter_anisotropic");
...
_SOKOL_PRIVATE bool _sg_gl_supported_texture_format(sg_pixel_format fmt)
{
	...
	case SG_PIXELFORMAT_DXT1:
	case SG_PIXELFORMAT_DXT3:
	case SG_PIXELFORMAT_DXT5: return _sg.gl.features[SG_FEATURE_TEXTURE_COMPRESSION_DXT];
	case SG_PIXELFORMAT_PVRTC2_RGB:
	case SG_PIXELFORMAT_PVRTC4_RGB:
	case SG_PIXELFORMAT_PVRTC2_RGBA:
	case SG_PIXELFORMAT_PVRTC4_RGBA:
	...
	case SG_PIXELFORMAT_ETC2_RGB8:
	case SG_PIXELFORMAT_ETC2_SRGB8: return _sg.gl.features[SG_FEATURE_TEXTURE_COMPRESSION_ETC2];
	default: return true;
	}
}
...
_SOKOL_PRIVATE sg_resource_state _sg_create_image(_sg_image_t * img, const sg_image_desc * desc)
{
	...
	if (_sg_is_valid_rendertarget_depth_format(img->pixel_format)) {
		...
		SOKOL_ASSERT(
		    !img->ext_textures); /* cannot provide external texture for depth images */
		...
		if (!_sg.gl.gles2 && msaa) {
			glRenderbufferStorageMultisample(
			    GL_RENDERBUFFER, img->sample_count, gl_depth_format, img->width,
			    img->height);
		} else
#endif
		{
			glRenderbufferStorage(
			    GL_RENDERBUFFER, gl_depth_format, img->width, img->height);
		}
	} else {
		/* regular color texture */
		img->gl_target = _sg_gl_texture_target(img->type);
		const GLenum gl_internal_format =
		    _sg_gl_teximage_internal_format(img->pixel_format);
...
/* if this is a MSAA render target, need to create a separate render buffer */
		...
		if (!_sg.gl.gles2 && img->render_target && msaa) {
			...
			glRenderbufferStorageMultisample(
			    GL_RENDERBUFFER, img->sample_count, gl_internal_format, img->width,
			    img->height);
		}
#endif
		...
		if (img->ext_textures) {
			...
			const bool   is_compressed =
			    _sg_is_compressed_pixel_format(img->pixel_format);
			for (int slot = 0; slot < img->num_slots; slot++) {
				...
				glTexParameteri(
				    img->gl_target, GL_TEXTURE_MIN_FILTER, gl_min_filter);
				glTexParameteri(
				    img->gl_target, GL_TEXTURE_MAG_FILTER, gl_mag_filter);
				if (_sg.gl.ext_anisotropic && (img->max_anisotropy > 1)) {
					...
					glTexParameteri(
					    img->gl_target, GL_TEXTURE_MAX_ANISOTROPY_EXT,
					    max_aniso);
				}
				if (img->type == SG_IMAGETYPE_CUBE) {
					glTexParameteri(
					    img->gl_target, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
					glTexParameteri(
					    img->gl_target, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
				} else {
					glTexParameteri(
					    img->gl_target, GL_TEXTURE_WRAP_S,
					    _sg_gl_wrap(img->wrap_u));
					glTexParameteri(
					    img->gl_target, GL_TEXTURE_WRAP_T,
					    _sg_gl_wrap(img->wrap_v));
#if !defined(SOKOL_GLES2)
					if (!_sg.gl.gles2 && (img->type == SG_IMAGETYPE_3D)) {
						glTexParameteri(
						    img->gl_target, GL_TEXTURE_WRAP_R,
...
#if !defined(SOKOL_GLES2)
				if (!_sg.gl.gles2) {
					/* GL spec has strange defaults for mipmap min/max lod:
					 * -1000 to +1000 */
					const float min_lod =
					    _sg_clamp(desc->min_lod, 0.0f, 1000.0f);
					const float max_lod = _sg_clamp(
					    _sg_def_flt(desc->max_lod, 1000.0f), 0.0f, 1000.0f);
					glTexParameterf(
					    img->gl_target, GL_TEXTURE_MIN_LOD, min_lod);
					glTexParameterf(
					    img->gl_target, GL_TEXTURE_MAX_LOD, max_lod);
				}
#endif
				...
				for (int face_index = 0; face_index < num_faces; face_index++) {
					for (int mip_index = 0; mip_index < img->num_mipmaps;
						...
						if (SG_IMAGETYPE_CUBE == img->type) {
							gl_img_target =
							    _sg_gl_cubeface_target(face_index);
						}
						const GLvoid * data_ptr =
						    desc->content.subimage[face_index][mip_index]
						        .ptr;
						const int data_size =
						    desc->content.subimage[face_index][mip_index]
						        .size;
						...
						if ((SG_IMAGETYPE_2D == img->type) ||
						    (SG_IMAGETYPE_CUBE == img->type)) {
							if (is_compressed) {
								glCompressedTexImage2D(
								    gl_img_target, mip_index,
								    gl_internal_format, mip_width,
								    mip_height, 0, data_size,
								    data_ptr);
							} else {
								const GLenum gl_type =
								    _sg_gl_teximage_type(
								        img->pixel_format);
								glTexImage2D(
								    gl_img_target, mip_index,
								    gl_internal_format, mip_width,
								    mip_height, 0, gl_format,
								    gl_type, data_ptr);
							}
						}
#if !defined(SOKOL_GLES2)
...
_SOKOL_PRIVATE void
_sg_gl_load_depth_stencil(const sg_depth_stencil_state * src, sg_depth_stencil_state * dst)
{
...
_SOKOL_PRIVATE void _sg_gl_load_blend(const sg_blend_state * src, sg_blend_state * dst)
{
	...
	if (src->color_write_mask == SG_COLORMASK_NONE) {
		...
		dst->color_write_mask =
		    (uint8_t)_sg_def((sg_color_mask)src->color_write_mask, SG_COLORMASK_RGBA);
	}
	for (int i = 0; i < 4; i++) {
...
_SOKOL_PRIVATE void
_sg_gl_load_rasterizer(const sg_rasterizer_state * src, sg_rasterizer_state * dst)
{
               ...
               _sg_create_pipeline(_sg_pipeline_t * pip, _sg_shader_t * shd, const sg_pipeline_desc * desc)
{
	...
	for (int attr_index = 0; attr_index < SG_MAX_VERTEX_ATTRIBUTES; attr_index++) {
		...
		const sg_vertex_step          step_func =
		    _sg_def(l_desc->step_func, SG_VERTEXSTEP_PER_VERTEX);
			...
			if (att_img) {
				...
				case SG_IMAGETYPE_2D:
					glFramebufferTexture2D(
					    GL_FRAMEBUFFER, gl_att, GL_TEXTURE_2D, gl_tex,
					    mip_level);
					break;
				case SG_IMAGETYPE_CUBE:
					glFramebufferTexture2D(
					    GL_FRAMEBUFFER, gl_att, _sg_gl_cubeface_target(slice),
					    gl_tex, mip_level);
					break;
				default:
/* 3D- or array-texture */
					...
					if (!_sg.gl.gles2) {
						glFramebufferTextureLayer(
						    GL_FRAMEBUFFER, gl_att, gl_tex, mip_level,
						    slice);
					}
#endif
					break;
	...
	if (pass->ds_att.image) {
		...
		SOKOL_ASSERT(gl_render_buffer);
		glFramebufferRenderbuffer(
		    GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, gl_render_buffer);
		...
		for (int i = 0; i < SG_MAX_COLOR_ATTACHMENTS; i++) {
			...
			if (att->image) {
				...
				default:
					...
					if (!_sg.gl.gles2) {
						glFramebufferTextureLayer(
						    GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, gl_tex,
				...
				if (glCheckFramebufferStatus(GL_FRAMEBUFFER) !=
					...
					SOKOL_LOG("Framebuffer completeness check failed (msaa "
					          "resolve buffer)!\n");
					return SG_RESOURCESTATE_FAILED;
				}
			}
	...
	if (!use_mrt_clear) {
		...
		if (pass->ds_att.image) {
			if ((action->depth.action == SG_ACTION_CLEAR) &&
				...
				glClearBufferfi(
				    GL_DEPTH_STENCIL, 0, action->depth.val, action->stencil.val);
	...
	if ((_sg.gl.cache.cur_pipeline != pip) ||
		...
		for (int i = 0; i < 2; i++) {
			...
			if ((new_ss->compare_func != cache_ss->compare_func) ||
				...
				glStencilFuncSeparate(
				    gl_face, _sg_gl_compare_func(new_ss->compare_func),
			...
			if ((new_ss->fail_op != cache_ss->fail_op) ||
				...
				glStencilOpSeparate(
				    gl_face, _sg_gl_stencil_op(new_ss->fail_op),
		...
		if ((new_b->op_rgb != cache_b->op_rgb) || (new_b->op_alpha != cache_b->op_alpha)) {
			...
			glBlendEquationSeparate(
			    _sg_gl_blend_op(new_b->op_rgb), _sg_gl_blend_op(new_b->op_alpha));
		}
		if (new_b->color_write_mask != cache_b->color_write_mask) {
			cache_b->color_write_mask = new_b->color_write_mask;
			glColorMask(
			    (new_b->color_write_mask & SG_COLORMASK_R) != 0,
		...
		if (new_r->cull_mode != cache_r->cull_mode) {
			...
			if (SG_CULLMODE_NONE == new_r->cull_mode) {
				...
				GLenum gl_mode =
				    (SG_CULLMODE_FRONT == new_r->cull_mode) ? GL_FRONT : GL_BACK;
				glCullFace(gl_mode);
			}
		}
		if (new_r->face_winding != cache_r->face_winding) {
			...
			GLenum gl_winding =
			    (SG_FACEWINDING_CW == new_r->face_winding) ? GL_CW : GL_CCW;
    ...
    _sg_image_t ** fs_imgs, int num_fs_imgs)
{
	...
	for (int stage_index = 0; stage_index < SG_NUM_SHADER_STAGES; stage_index++) {
		...
		SOKOL_ASSERT(
		    ((stage_index == SG_SHADERSTAGE_VS) ? num_vs_imgs : num_fs_imgs) ==
		    stage->num_images);
		...
		if (attr->vb_index >= 0) {
			...
			if ((gl_vb != cache_attr->gl_vbuf) ||
				...
				glVertexAttribPointer(
				    attr_index, attr->size, attr->type, attr->normalized,
...
_sg_apply_uniforms(sg_shader_stage stage_index, int ub_index, const void * data, int num_bytes)
{
	...
	SOKOL_ASSERT(
	    _sg.gl.cache.cur_pipeline->shader->slot.id == _sg.gl.cache.cur_pipeline->shader_id.id);
	...
	for (int u_index = 0; u_index < ub->num_uniforms; u_index++) {
		...
		switch (u->type) {
		case SG_UNIFORMTYPE_INVALID: break;
		case SG_UNIFORMTYPE_FLOAT: glUniform1fv(u->gl_loc, u->count, ptr); break;
		case SG_UNIFORMTYPE_FLOAT2: glUniform2fv(u->gl_loc, u->count, ptr); break;
		case SG_UNIFORMTYPE_FLOAT3: glUniform3fv(u->gl_loc, u->count, ptr); break;
		case SG_UNIFORMTYPE_FLOAT4: glUniform4fv(u->gl_loc, u->count, ptr); break;
		...
		if (num_instances == 1) {
			...
			if (_sg.gl.features[SG_FEATURE_INSTANCING]) {
				glDrawElementsInstanced(
				    p_type, num_elements, i_type, indices, num_instances);
			}
		}
	} else {
		...
		if (num_instances == 1) {
			...
			if (_sg.gl.features[SG_FEATURE_INSTANCING]) {
				glDrawArraysInstanced(
				    p_type, base_element, num_elements, num_instances);
			}
		}
	}
}
...
_SOKOL_PRIVATE void
_sg_append_buffer(_sg_buffer_t * buf, const void * data_ptr, int data_size, bool new_frame)
{
	SOKOL_ASSERT(buf && data_ptr && (data_size > 0));
	if (new_frame) {
		if (++buf->active_slot >= buf->num_slots) {
	...
	for (int face_index = 0; face_index < num_faces; face_index++) {
		for (int mip_index = 0; mip_index < num_mips; mip_index++) {
			...
			if ((SG_IMAGETYPE_2D == img->type) || (SG_IMAGETYPE_CUBE == img->type)) {
				glTexSubImage2D(
				    gl_img_target, mip_index, 0, 0, mip_width, mip_height,
				...
				glTexSubImage3D(
				    gl_img_target, mip_index, 0, 0, 0, mip_width, mip_height,
				    mip_depth, gl_img_format, gl_img_type, data_ptr);
			}
#endif
		}
	}
...
{
	switch (usg) {
	case SG_USAGE_IMMUTABLE: return D3D11_USAGE_IMMUTABLE;
	case SG_USAGE_DYNAMIC:
	case SG_USAGE_STREAM: return D3D11_USAGE_DYNAMIC;
	default: SOKOL_UNREACHABLE; return (D3D11_USAGE)0;
	}
}
...
{
	switch (usg) {
	case SG_USAGE_IMMUTABLE: return 0;
	case SG_USAGE_DYNAMIC:
	case SG_USAGE_STREAM: return D3D11_CPU_ACCESS_WRITE;
	default: SOKOL_UNREACHABLE; return 0;
	}
}
...
_SOKOL_PRIVATE D3D11_FILTER
               _sg_d3d11_filter(sg_filter min_f, sg_filter mag_f, uint32_t max_anisotropy)
{
	if (max_anisotropy > 1) {
		...
		case SG_FILTER_NEAREST_MIPMAP_NEAREST: return D3D11_FILTER_MIN_MAG_MIP_POINT;
		case SG_FILTER_LINEAR:
		case SG_FILTER_LINEAR_MIPMAP_NEAREST: return D3D11_FILTER_MIN_LINEAR_MAG_MIP_POINT;
		case SG_FILTER_NEAREST_MIPMAP_LINEAR: return D3D11_FILTER_MIN_MAG_POINT_MIP_LINEAR;
		...
		case SG_FILTER_LINEAR:
		case SG_FILTER_LINEAR_MIPMAP_NEAREST: return D3D11_FILTER_MIN_MAG_LINEAR_MIP_POINT;
		case SG_FILTER_NEAREST_MIPMAP_LINEAR: return D3D11_FILTER_MIN_POINT_MAG_MIP_LINEAR;
		case SG_FILTER_LINEAR_MIPMAP_LINEAR: return D3D11_FILTER_MIN_MAG_MIP_LINEAR;
		default: SOKOL_UNREACHABLE; break;
		}
	}
...
_SOKOL_PRIVATE bool _sg_query_feature(sg_feature f)
{
	...
	case SG_FEATURE_IMAGETYPE_ARRAY: return true;
	default: return false;
	}
}
...
{
	/* clear all the device context state, so that resource refs don't keep stuck in the d3d
	 * device context */
	ID3D11DeviceContext_OMSetRenderTargets(
	    _sg.d3d11.ctx, SG_MAX_COLOR_ATTACHMENTS, _sg.d3d11.zero_rtvs, NULL);
	...
	ID3D11DeviceContext_IASetVertexBuffers(
	    _sg.d3d11.ctx, 0, SG_MAX_SHADERSTAGE_BUFFERS, _sg.d3d11.zero_vbs,
	    _sg.d3d11.zero_vb_strides, _sg.d3d11.zero_vb_offsets);
	...
	ID3D11DeviceContext_VSSetConstantBuffers(
	    _sg.d3d11.ctx, 0, SG_MAX_SHADERSTAGE_UBS, _sg.d3d11.zero_cbs);
	ID3D11DeviceContext_PSSetConstantBuffers(
	    _sg.d3d11.ctx, 0, SG_MAX_SHADERSTAGE_UBS, _sg.d3d11.zero_cbs);
	ID3D11DeviceContext_VSSetShaderResources(
	    _sg.d3d11.ctx, 0, SG_MAX_SHADERSTAGE_IMAGES, _sg.d3d11.zero_srvs);
	ID3D11DeviceContext_PSSetShaderResources(
	    _sg.d3d11.ctx, 0, SG_MAX_SHADERSTAGE_IMAGES, _sg.d3d11.zero_srvs);
	ID3D11DeviceContext_VSSetSamplers(
	    _sg.d3d11.ctx, 0, SG_MAX_SHADERSTAGE_IMAGES, _sg.d3d11.zero_smps);
...
_SOKOL_PRIVATE sg_resource_state _sg_create_buffer(_sg_buffer_t * buf, const sg_buffer_desc * desc)
{
	...
	if (injected) {
		...
		d3d11_desc.BindFlags = buf->type == SG_BUFFERTYPE_VERTEXBUFFER
		...
		HRESULT hr = ID3D11Device_CreateBuffer(
		    _sg.d3d11.dev, &d3d11_desc, init_data_ptr, &buf->d3d11_buf);
		_SOKOL_UNUSED(hr);
...
_SOKOL_PRIVATE void
_sg_d3d11_fill_subres_data(const _sg_image_t * img, const sg_image_content * content)
{
	const int num_faces    = (img->type == SG_IMAGETYPE_CUBE) ? 6 : 1;
	...
	for (int face_index = 0; face_index < num_faces; face_index++) {
		for (int slice_index = 0; slice_index < num_slices; slice_index++) {
			for (int mip_index = 0; mip_index < img->num_mipmaps;
				...
				SOKOL_ASSERT(
				    subres_index < (SG_MAX_MIPMAPS * SG_MAX_TEXTUREARRAY_LAYERS));
				D3D11_SUBRESOURCE_DATA * subres_data =
				    &_sg.d3d11.subres_data[subres_index];
				const int mip_width =
				    ((img->width >> mip_index) > 0) ? img->width >> mip_index : 1;
				...
				subres_data->SysMemPitch =
				    _sg_row_pitch(img->pixel_format, mip_width);
				if (img->type == SG_IMAGETYPE_3D) {
					/* FIXME? const int mip_depth = ((img->depth>>mip_index)>0)
					 * ? img->depth>>mip_index : 1; */
					subres_data->SysMemSlicePitch = _sg_surface_pitch(
					    img->pixel_format, mip_width, mip_height);
				} else {
...
_SOKOL_PRIVATE sg_resource_state _sg_create_image(_sg_image_t * img, const sg_image_desc * desc)
{
	SOKOL_ASSERT(img && desc);
	SOKOL_ASSERT(
	    !img->d3d11_tex2d && !img->d3d11_tex3d && !img->d3d11_texds && !img->d3d11_texmsaa);
	...
	if (_sg_is_valid_rendertarget_depth_format(img->pixel_format)) {
		...
		if (img->d3d11_format == DXGI_FORMAT_UNKNOWN) {
			...
			SOKOL_LOG("trying to create a D3D11 depth-texture with unsupported pixel "
			          "format\n");
			return SG_RESOURCESTATE_FAILED;
		}
		...
		d3d11_desc.SampleDesc.Quality =
		    (img->sample_count > 1) ? D3D11_STANDARD_MULTISAMPLE_PATTERN : 0;
		...
		if (img->type != SG_IMAGETYPE_3D) {
			...
			/* if this is an MSAA render target, the following texture will be the
			 * 'resolve-texture' */
			...
			if (img->render_target) {
				img->d3d11_format =
				    _sg_d3d11_rendertarget_color_format(img->pixel_format);
				...
				d3d11_tex_desc.CPUAccessFlags =
				    _sg_d3d11_cpu_access_flags(img->usage);
			}
			if (img->d3d11_format == DXGI_FORMAT_UNKNOWN) {
				/* trying to create a texture format that's not supported by D3D */
				SOKOL_LOG("trying to create a D3D11 texture with unsupported pixel "
				          "format\n");
				return SG_RESOURCESTATE_FAILED;
			}
			...
			if (injected) {
				...
				hr = ID3D11Device_CreateTexture2D(
				    _sg.d3d11.dev, &d3d11_tex_desc, init_data, &img->d3d11_tex2d);
			...
			if (img->sample_count > 1) {
				...
				hr = ID3D11Device_CreateTexture2D(
				    _sg.d3d11.dev, &d3d11_tex_desc, NULL, &img->d3d11_texmsaa);
			...
			hr = ID3D11Device_CreateShaderResourceView(
			    _sg.d3d11.dev, (ID3D11Resource *)img->d3d11_tex2d, &d3d11_srv_desc,
			    &img->d3d11_srv);
			...
			if (injected) {
				...
				hr = ID3D11Device_CreateTexture3D(
				    _sg.d3d11.dev, &d3d11_tex_desc, init_data, &img->d3d11_tex3d);
			...
			hr                                 = ID3D11Device_CreateShaderResourceView(
                            _sg.d3d11.dev, (ID3D11Resource *)img->d3d11_tex3d, &d3d11_srv_desc,
                            &img->d3d11_srv);
			SOKOL_ASSERT(SUCCEEDED(hr) && img->d3d11_srv);
		}

		/* sampler state object, note D3D11 implements an internal shared-pool for sampler
		 * objects */
		...
		hr                            = ID3D11Device_CreateSamplerState(
                    _sg.d3d11.dev, &d3d11_smp_desc, &img->d3d11_smp);
...
#if defined(SOKOL_D3D11_SHADER_COMPILER)
_SOKOL_PRIVATE ID3DBlob *
               _sg_d3d11_compile_shader(const sg_shader_stage_desc * stage_desc, const char * target)
{
	...
	for (int stage_index = 0; stage_index < SG_NUM_SHADER_STAGES; stage_index++) {
		...
		for (int ub_index = 0; ub_index < SG_MAX_SHADERSTAGE_UBS; ub_index++) {
			...
			hr                = ID3D11Device_CreateBuffer(
                            _sg.d3d11.dev, &cb_desc, NULL, &stage->d3d11_cbs[ub_index]);
...
/* compile shader code */
	...
	if (vs_ptr && fs_ptr && (vs_length > 0) && (fs_length > 0)) {
		...
		hr = ID3D11Device_CreateVertexShader(
		    _sg.d3d11.dev, vs_ptr, vs_length, NULL, &shd->d3d11_vs);
		SOKOL_ASSERT(SUCCEEDED(hr) && shd->d3d11_vs);
		hr = ID3D11Device_CreatePixelShader(
		    _sg.d3d11.dev, fs_ptr, fs_length, NULL, &shd->d3d11_fs);
		SOKOL_ASSERT(SUCCEEDED(hr) && shd->d3d11_fs);

		/* need to store the vertex shader byte code, this is needed later in
		 * sg_create_pipeline */
               ...
               _sg_create_pipeline(_sg_pipeline_t * pip, _sg_shader_t * shd, const sg_pipeline_desc * desc)
{
	...
	pip->d3d11_topology =
	    _sg_d3d11_primitive_topology(_sg_def(desc->primitive_type, SG_PRIMITIVETYPE_TRIANGLES));
	for (int i = 0; i < 4; i++) {
		pip->blend_color[i] = desc->blend.blend_color[i];
	}
	...
	for (int layout_index = 0; layout_index < SG_MAX_SHADERSTAGE_BUFFERS; layout_index++) {
		if (pip->vertex_layout_valid[layout_index]) {
			...
			const int                     stride =
			    l_desc->stride ? l_desc->stride : auto_offset[layout_index];
	...
	hr = ID3D11Device_CreateInputLayout(
	    _sg.d3d11.dev, d3d11_comps, /* pInputElementDesc */
	...
	rs_desc.CullMode =
	    _sg_d3d11_cull_mode(_sg_def(desc->rasterizer.cull_mode, SG_CULLMODE_NONE));
	rs_desc.FrontCounterClockwise =
	    _sg_def(desc->rasterizer.face_winding, SG_FACEWINDING_CW) == SG_FACEWINDING_CCW;
	...
	dss_desc.DepthWriteMask = desc->depth_stencil.depth_write_enabled
	                              ? D3D11_DEPTH_WRITE_MASK_ALL
	                              : D3D11_DEPTH_WRITE_MASK_ZERO;
	dss_desc.DepthFunc = _sg_d3d11_compare_func(
	    _sg_def(desc->depth_stencil.depth_compare_func, SG_COMPAREFUNC_ALWAYS));
	...
	dss_desc.FrontFace.StencilFailOp =
	    _sg_d3d11_stencil_op(_sg_def(sf->fail_op, SG_STENCILOP_KEEP));
	dss_desc.FrontFace.StencilDepthFailOp =
	    _sg_d3d11_stencil_op(_sg_def(sf->depth_fail_op, SG_STENCILOP_KEEP));
	dss_desc.FrontFace.StencilPassOp =
	    _sg_d3d11_stencil_op(_sg_def(sf->pass_op, SG_STENCILOP_KEEP));
	dss_desc.FrontFace.StencilFunc =
	    _sg_d3d11_compare_func(_sg_def(sf->compare_func, SG_COMPAREFUNC_ALWAYS));
	const sg_stencil_state * sb = &desc->depth_stencil.stencil_back;
	dss_desc.BackFace.StencilFailOp =
	    _sg_d3d11_stencil_op(_sg_def(sb->fail_op, SG_STENCILOP_KEEP));
	dss_desc.BackFace.StencilDepthFailOp =
	    _sg_d3d11_stencil_op(_sg_def(sb->depth_fail_op, SG_STENCILOP_KEEP));
	dss_desc.BackFace.StencilPassOp =
	    _sg_d3d11_stencil_op(_sg_def(sb->pass_op, SG_STENCILOP_KEEP));
	dss_desc.BackFace.StencilFunc =
	    _sg_d3d11_compare_func(_sg_def(sb->compare_func, SG_COMPAREFUNC_ALWAYS));
	...
	bs_desc.RenderTarget[0].SrcBlend =
	    _sg_d3d11_blend_factor(_sg_def(desc->blend.src_factor_rgb, SG_BLENDFACTOR_ONE));
	bs_desc.RenderTarget[0].DestBlend =
	    _sg_d3d11_blend_factor(_sg_def(desc->blend.dst_factor_rgb, SG_BLENDFACTOR_ZERO));
	bs_desc.RenderTarget[0].BlendOp =
	    _sg_d3d11_blend_op(_sg_def(desc->blend.op_rgb, SG_BLENDOP_ADD));
	bs_desc.RenderTarget[0].SrcBlendAlpha =
	    _sg_d3d11_blend_factor(_sg_def(desc->blend.src_factor_alpha, SG_BLENDFACTOR_ONE));
	bs_desc.RenderTarget[0].DestBlendAlpha =
	    _sg_d3d11_blend_factor(_sg_def(desc->blend.dst_factor_alpha, SG_BLENDFACTOR_ZERO));
	bs_desc.RenderTarget[0].BlendOpAlpha =
	    _sg_d3d11_blend_op(_sg_def(desc->blend.op_alpha, SG_BLENDOP_ADD));
	bs_desc.RenderTarget[0].RenderTargetWriteMask = _sg_d3d11_color_write_mask(
	    _sg_def((sg_color_mask)desc->blend.color_write_mask, SG_COLORMASK_RGBA));
               ...
               _sg_create_pass(_sg_pass_t * pass, _sg_image_t ** att_images, const sg_pass_desc * desc)
{
		...
		if (att_desc->image.id != SG_INVALID_ID) {
			...
			case SG_IMAGETYPE_ARRAY:
				if (is_msaa) {
					...
					d3d11_rtv_desc.Texture2DMSArray.FirstArraySlice =
					    att->slice;
			...
			HRESULT hr = ID3D11Device_CreateRenderTargetView(
			    _sg.d3d11.dev, d3d11_res, &d3d11_rtv_desc, &pass->d3d11_rtvs[i]);
			_SOKOL_UNUSED(hr);
	...
	if (att_desc->image.id != SG_INVALID_ID) {
		...
		HRESULT hr = ID3D11Device_CreateDepthStencilView(
		    _sg.d3d11.dev, d3d11_res, &d3d11_dsv_desc, &pass->d3d11_dsv);
		_SOKOL_UNUSED(hr);
...
_SOKOL_PRIVATE void _sg_begin_pass(_sg_pass_t * pass, const sg_pass_action * action, int w, int h)
{
	...
	ID3D11DeviceContext_OMSetRenderTargets(
	    _sg.d3d11.ctx, SG_MAX_COLOR_ATTACHMENTS, _sg.d3d11.cur_rtvs, _sg.d3d11.cur_dsv);
	...
	for (int i = 0; i < _sg.d3d11.num_rtvs; i++) {
		if (action->colors[i].action == SG_ACTION_CLEAR) {
			ID3D11DeviceContext_ClearRenderTargetView(
			    _sg.d3d11.ctx, _sg.d3d11.cur_rtvs[i], action->colors[i].val);
		}
	}
	UINT ds_flags = 0;
	if (action->depth.action == SG_ACTION_CLEAR) {
	...
	if ((0 != ds_flags) && _sg.d3d11.cur_dsv) {
		ID3D11DeviceContext_ClearDepthStencilView(
		    _sg.d3d11.ctx, _sg.d3d11.cur_dsv, ds_flags, action->depth.val,
		    action->stencil.val);
	}
}
	...
	if (_sg.d3d11.cur_pass) {
		...
		for (int i = 0; i < _sg.d3d11.num_rtvs; i++) {
			...
			if (att->image->sample_count > 1) {
				SOKOL_ASSERT(
				    att->image->d3d11_tex2d && att->image->d3d11_texmsaa &&
				    !att->image->d3d11_tex3d);
				SOKOL_ASSERT(DXGI_FORMAT_UNKNOWN != att->image->d3d11_format);
				const _sg_image_t * img    = att->image;
				UINT                subres = _sg_d3d11_calcsubresource(
                                    att->mip_level, att->slice, img->num_mipmaps);
...
_SOKOL_PRIVATE void _sg_apply_pipeline(_sg_pipeline_t * pip)
{
	...
	ID3D11DeviceContext_OMSetDepthStencilState(
	    _sg.d3d11.ctx, pip->d3d11_dss, pip->d3d11_stencil_ref);
	ID3D11DeviceContext_OMSetBlendState(
	    _sg.d3d11.ctx, pip->d3d11_bs, pip->blend_color, 0xFFFFFFFF);
	...
	ID3D11DeviceContext_VSSetConstantBuffers(
	    _sg.d3d11.ctx, 0, SG_MAX_SHADERSTAGE_UBS,
    ...
    _sg_image_t ** fs_imgs, int num_fs_imgs)
{
	...
	ID3D11DeviceContext_IASetVertexBuffers(
	    _sg.d3d11.ctx, 0, SG_MAX_SHADERSTAGE_BUFFERS, d3d11_vbs, pip->d3d11_vb_strides,
	    d3d11_vb_offsets);
	ID3D11DeviceContext_IASetIndexBuffer(
	    _sg.d3d11.ctx, d3d11_ib, pip->d3d11_index_format, ib_offset);
	ID3D11DeviceContext_VSSetShaderResources(
	    _sg.d3d11.ctx, 0, SG_MAX_SHADERSTAGE_IMAGES, d3d11_vs_srvs);
	ID3D11DeviceContext_VSSetSamplers(
	    _sg.d3d11.ctx, 0, SG_MAX_SHADERSTAGE_IMAGES, d3d11_vs_smps);
	ID3D11DeviceContext_PSSetShaderResources(
	    _sg.d3d11.ctx, 0, SG_MAX_SHADERSTAGE_IMAGES, d3d11_fs_srvs);
	ID3D11DeviceContext_PSSetSamplers(
	    _sg.d3d11.ctx, 0, SG_MAX_SHADERSTAGE_IMAGES, d3d11_fs_smps);
}
...
_sg_apply_uniforms(sg_shader_stage stage_index, int ub_index, const void * data, int num_bytes)
{
	...
	ID3D11DeviceContext_UpdateSubresource(
	    _sg.d3d11.ctx, (ID3D11Resource *)cb, 0, NULL, data, 0, 0);
}
	...
	if (_sg.d3d11.use_indexed_draw) {
		if (1 == num_instances) {
			ID3D11DeviceContext_DrawIndexed(
			    _sg.d3d11.ctx, num_elements, base_element, 0);
		} else {
			ID3D11DeviceContext_DrawIndexedInstanced(
			    _sg.d3d11.ctx, num_elements, num_instances, base_element, 0, 0);
		}
	} else {
		if (1 == num_instances) {
			...
			ID3D11DeviceContext_DrawInstanced(
			    _sg.d3d11.ctx, num_elements, num_instances, base_element, 0);
		}
	}
}
...
_SOKOL_PRIVATE void _sg_update_buffer(_sg_buffer_t * buf, const void * data_ptr, int data_size)
{
	...
	HRESULT                  hr = ID3D11DeviceContext_Map(
            _sg.d3d11.ctx, (ID3D11Resource *)buf->d3d11_buf, 0, D3D11_MAP_WRITE_DISCARD, 0,
            &d3d11_msr);
	_SOKOL_UNUSED(hr);
...
_sg_append_buffer(_sg_buffer_t * buf, const void * data_ptr, int data_size, bool new_frame)
{
	...
	D3D11_MAPPED_SUBRESOURCE d3d11_msr;
	HRESULT                  hr = ID3D11DeviceContext_Map(
            _sg.d3d11.ctx, (ID3D11Resource *)buf->d3d11_buf, 0, map_type, 0, &d3d11_msr);
	_SOKOL_UNUSED(hr);
	SOKOL_ASSERT(SUCCEEDED(hr));
		...
		for (int slice_index = 0; slice_index < num_slices; slice_index++) {
			for (int mip_index = 0; mip_index < img->num_mipmaps;
				...
				const uint8_t * slice_ptr =
				    ((const uint8_t *)subimg_content->ptr) + slice_offset;
				hr = ID3D11DeviceContext_Map(
				    _sg.d3d11.ctx, d3d11_res, subres_index, D3D11_MAP_WRITE_DISCARD,
				    0, &d3d11_msr);
				SOKOL_ASSERT(SUCCEEDED(hr));
				/* FIXME: need to handle difference in depth-pitch for 3D textures
				 * as well! */
				if (src_pitch == (int)d3d11_msr.RowPitch) {
					...
					for (int row_index = 0; row_index < mip_height;
					     row_index++) {
...
{
	switch (usg) {
	case SG_USAGE_IMMUTABLE: return MTLResourceStorageModeShared;
	case SG_USAGE_DYNAMIC:
...
{
	switch (fmt) {
	case SG_PIXELFORMAT_DEPTHSTENCIL: return MTLPixelFormatDepth32Float_Stencil8;
	default: return MTLPixelFormatInvalid;
	}
}
...
_SOKOL_PRIVATE bool _sg_mtl_is_pvrtc(sg_pixel_format fmt)
{
	...
	case SG_PIXELFORMAT_PVRTC2_RGBA:
	case SG_PIXELFORMAT_PVRTC4_RGB:
	case SG_PIXELFORMAT_PVRTC4_RGBA: return true;
	default: return false;
	}
}
...
_SOKOL_PRIVATE MTLSamplerMinMagFilter _sg_mtl_minmag_filter(sg_filter f)
{
	...
	case SG_FILTER_NEAREST:
	case SG_FILTER_NEAREST_MIPMAP_NEAREST:
	case SG_FILTER_NEAREST_MIPMAP_LINEAR: return MTLSamplerMinMagFilterNearest;
	case SG_FILTER_LINEAR:
	case SG_FILTER_LINEAR_MIPMAP_NEAREST:
	case SG_FILTER_LINEAR_MIPMAP_LINEAR: return MTLSamplerMinMagFilterLinear;
	default: SOKOL_UNREACHABLE; return (MTLSamplerMinMagFilter)0;
	}
}
...
_SOKOL_PRIVATE MTLSamplerMipFilter _sg_mtl_mip_filter(sg_filter f)
{
	switch (f) {
	case SG_FILTER_NEAREST:
	case SG_FILTER_LINEAR: return MTLSamplerMipFilterNotMipmapped;
	case SG_FILTER_NEAREST_MIPMAP_NEAREST:
	case SG_FILTER_LINEAR_MIPMAP_NEAREST: return MTLSamplerMipFilterNearest;
	case SG_FILTER_NEAREST_MIPMAP_LINEAR:
	case SG_FILTER_LINEAR_MIPMAP_LINEAR: return MTLSamplerMipFilterLinear;
	default: SOKOL_UNREACHABLE; return (MTLSamplerMipFilter)0;
	}
}
...
_SOKOL_PRIVATE void _sg_mtl_init_pool(const sg_desc * desc)
{
	_sg.mtl.idpool.num_slots =
	    2 * (2 * _sg_def(desc->buffer_pool_size, _SG_DEFAULT_BUFFER_POOL_SIZE) +
...
{
	_sg.mtl.sampler_cache.capacity =
	    _sg_def(desc->mtl_sampler_cache_size, _SG_MTL_DEFAULT_SAMPLER_CACHE_CAPACITY);
...
_SOKOL_PRIVATE void _sg_mtl_destroy_sampler_cache(uint32_t frame_index)
{
	...
	for (int i = 0; i < _sg.mtl.sampler_cache.num_items; i++) {
		_sg_mtl_release_resource(
		    frame_index, _sg.mtl.sampler_cache.items[i].mtl_sampler_state);
	}
...
_SOKOL_PRIVATE uint32_t
               _sg_mtl_create_sampler(id<MTLDevice> mtl_device, const sg_image_desc * img_desc)
{
	...
	/* first try to find identical sampler, number of samplers will be small, so linear search
	 * is ok */
	for (int i = 0; i < _sg.mtl.sampler_cache.num_items; i++) {
		_sg_mtl_sampler_cache_item_t * item = &_sg.mtl.sampler_cache.items[i];
		if ((min_filter == item->min_filter) && (mag_filter == item->mag_filter) &&
		    (wrap_u == item->wrap_u) && (wrap_v == item->wrap_v) &&
		    (wrap_w == item->wrap_w) && (max_anisotropy == item->max_anisotropy) &&
...
_SOKOL_PRIVATE void _sg_setup_backend(const sg_desc * desc)
{
	...
	for (int i = 0; i < SG_NUM_INFLIGHT_FRAMES; i++) {
		_sg_mtl_uniform_buffers[i] = [_sg_mtl_device newBufferWithLength:_sg.mtl.ub_size
		                                                         options:res_opts];
	}
}
	...
	for (int slot = 0; slot < buf->num_slots; slot++) {
		...
		if (injected) {
			...
			if (buf->usage == SG_USAGE_IMMUTABLE) {
				...
				mtl_buf = [_sg_mtl_device newBufferWithLength:buf->size
				                                      options:mtl_options];
			}
		}
...
_SOKOL_PRIVATE void _sg_mtl_copy_image_content(
	...
	for (int face_index = 0; face_index < num_faces; face_index++) {
		for (int mip_index = 0; mip_index < img->num_mipmaps; mip_index++) {
			...
			const uint8_t * data_ptr =
			    (const uint8_t *)content->subimage[face_index][mip_index].ptr;
...
_SOKOL_PRIVATE id<MTLLibrary> _sg_mtl_compile_library(const char * src)
{
	NSError *      err = NULL;
	id<MTLLibrary> lib =
	    [_sg_mtl_device newLibraryWithSource:[NSString stringWithUTF8String:src]
	                                 options:nil
	                                   error:&err];
	if (err) {
               ...
               _sg_create_pipeline(_sg_pipeline_t * pip, _sg_shader_t * shd, const sg_pipeline_desc * desc)
{
	...
	pip->mtl_cull_mode =
	    _sg_mtl_cull_mode(_sg_def(desc->rasterizer.cull_mode, SG_CULLMODE_NONE));
	pip->mtl_winding =
	    _sg_mtl_winding(_sg_def(desc->rasterizer.face_winding, SG_FACEWINDING_CW));
	...
	for (int attr_index = 0; attr_index < SG_MAX_VERTEX_ATTRIBUTES; attr_index++) {
		...
		vtx_desc.attributes[attr_index].bufferIndex =
		    a_desc->buffer_index + SG_MAX_SHADERSTAGE_UBS;
		auto_offset[a_desc->buffer_index] += _sg_vertexformat_bytesize(a_desc->format);
		pip->vertex_layout_valid[a_desc->buffer_index] = true;
	}
	for (int layout_index = 0; layout_index < SG_MAX_SHADERSTAGE_BUFFERS; layout_index++) {
		if (pip->vertex_layout_valid[layout_index]) {
			...
			vtx_desc.layouts[mtl_vb_slot].stepFunction = _sg_mtl_step_function(
			    _sg_def(l_desc->step_func, SG_VERTEXSTEP_PER_VERTEX));
	...
	*/
	...
	for (int i = 0; i < att_count; i++) {
		...
		rp_desc.colorAttachments[i].destinationAlphaBlendFactor = _sg_mtl_blend_factor(
		    _sg_def(desc->blend.dst_factor_alpha, SG_BLENDFACTOR_ZERO));
	...
	ds_desc.depthCompareFunction        = _sg_mtl_compare_func(
            _sg_def(desc->depth_stencil.depth_compare_func, SG_COMPAREFUNC_ALWAYS));
	...
	if (desc->depth_stencil.stencil_enabled) {
		...
		ds_desc.backFaceStencil.stencilFailureOperation =
		    _sg_mtl_stencil_op(_sg_def(sb->fail_op, SG_STENCILOP_KEEP));
		ds_desc.backFaceStencil.depthFailureOperation =
		    _sg_mtl_stencil_op(_sg_def(sb->depth_fail_op, SG_STENCILOP_KEEP));
		ds_desc.backFaceStencil.depthStencilPassOperation =
		    _sg_mtl_stencil_op(_sg_def(sb->pass_op, SG_STENCILOP_KEEP));
		ds_desc.backFaceStencil.stencilCompareFunction =
		    _sg_mtl_compare_func(_sg_def(sb->compare_func, SG_COMPAREFUNC_ALWAYS));
		...
		ds_desc.frontFaceStencil.stencilFailureOperation =
		    _sg_mtl_stencil_op(_sg_def(sf->fail_op, SG_STENCILOP_KEEP));
		ds_desc.frontFaceStencil.depthFailureOperation =
		    _sg_mtl_stencil_op(_sg_def(sf->depth_fail_op, SG_STENCILOP_KEEP));
		ds_desc.frontFaceStencil.depthStencilPassOperation =
		    _sg_mtl_stencil_op(_sg_def(sf->pass_op, SG_STENCILOP_KEEP));
	...
	id<MTLDepthStencilState> mtl_dss =
	    [_sg_mtl_device newDepthStencilStateWithDescriptor:ds_desc];
...
_SOKOL_PRIVATE void _sg_begin_pass(_sg_pass_t * pass, const sg_pass_action * action, int w, int h)
{
	...
	if (pass) {
		...
		/* default render pass, call user-provided callback to provide render pass
		 * descriptor */
	...
	if (pass) {
		...
		for (int i = 0; i < SG_MAX_COLOR_ATTACHMENTS; i++) {
			...
			pass_desc.colorAttachments[i].clearColor =
			    MTLClearColorMake(c[0], c[1], c[2], c[3]);
			if (is_msaa) {
				SOKOL_ASSERT(
				    att->image->mtl_msaa_tex != _SG_MTL_INVALID_SLOT_INDEX);
				SOKOL_ASSERT(
				    att->image->mtl_tex[att->image->active_slot] !=
				    _SG_MTL_INVALID_SLOT_INDEX);
				pass_desc.colorAttachments[i].texture =
				    _sg_mtl_idpool[att->image->mtl_msaa_tex];
		...
		if (0 != pass->ds_att.image) {
			...
			pass_desc.depthAttachment.texture =
			    _sg_mtl_idpool[att->image->mtl_depth_tex];
			pass_desc.depthAttachment.loadAction =
			    _sg_mtl_load_action(action->depth.action);
		...
		pass_desc.colorAttachments[0].loadAction =
		    _sg_mtl_load_action(action->colors[0].action);
    ...
    _sg_image_t ** fs_imgs, int num_fs_imgs)
{
	...
	for (slot = 0; slot < num_vbs; slot++) {
		...
		if ((_sg.mtl.state_cache.cur_vertexbuffers[slot] != vb) ||
			...
			[_sg_mtl_cmd_encoder
			    setVertexBuffer:_sg_mtl_idpool[vb->mtl_buf[vb->active_slot]]
	...
	for (slot = 0; slot < num_vs_imgs; slot++) {
		...
		if ((_sg.mtl.state_cache.cur_vs_images[slot] != img) ||
			...
			[_sg_mtl_cmd_encoder
			    setVertexTexture:_sg_mtl_idpool[img->mtl_tex[img->active_slot]]
			             atIndex:slot];
			SOKOL_ASSERT(img->mtl_sampler_state != _SG_MTL_INVALID_SLOT_INDEX);
			[_sg_mtl_cmd_encoder
			    setVertexSamplerState:_sg_mtl_idpool[img->mtl_sampler_state]
			                  atIndex:slot];
		}
	}
	...
	for (slot = 0; slot < num_fs_imgs; slot++) {
		...
		if ((_sg.mtl.state_cache.cur_fs_images[slot] != img) ||
			...
			SOKOL_ASSERT(img->mtl_tex[img->active_slot] != _SG_MTL_INVALID_SLOT_INDEX);
			[_sg_mtl_cmd_encoder
			    setFragmentTexture:_sg_mtl_idpool[img->mtl_tex[img->active_slot]]
			               atIndex:slot];
			SOKOL_ASSERT(img->mtl_sampler_state != _SG_MTL_INVALID_SLOT_INDEX);
			[_sg_mtl_cmd_encoder
			    setFragmentSamplerState:_sg_mtl_idpool[img->mtl_sampler_state]
			                    atIndex:slot];
		}
	}
}
...
_sg_apply_uniforms(sg_shader_stage stage_index, int ub_index, const void * data, int num_bytes)
{
	...
	SOKOL_ASSERT(
	    _sg.mtl.state_cache.cur_pipeline->shader->slot.id ==
	...
	SOKOL_ASSERT(
	    num_bytes <= _sg.mtl.state_cache.cur_pipeline->shader->stage[stage_index]
	...
	if (stage_index == SG_SHADERSTAGE_VS) {
		...
		[_sg_mtl_cmd_encoder setFragmentBufferOffset:_sg.mtl.cur_ub_offset
		                                     atIndex:ub_index];
	}
	_sg.mtl.cur_ub_offset =
	    _sg_mtl_roundup(_sg.mtl.cur_ub_offset + num_bytes, _SG_MTL_UB_ALIGN);
}
...
_SOKOL_PRIVATE void _sg_draw(int base_element, int num_elements, int num_instances)
{
	...
	if (SG_INDEXTYPE_NONE != _sg.mtl.state_cache.cur_pipeline->index_type) {
		...
		[_sg_mtl_cmd_encoder
		    drawIndexedPrimitives:_sg.mtl.state_cache.cur_pipeline->mtl_prim_type
...
_SOKOL_PRIVATE void _sg_setup_pools(_sg_pools_t * p, const sg_desc * desc)
{
	...
	_sg_init_pool(
	    &p->buffer_pool, _sg_def(desc->buffer_pool_size, _SG_DEFAULT_BUFFER_POOL_SIZE));
	...
	_sg_init_pool(
	    &p->shader_pool, _sg_def(desc->shader_pool_size, _SG_DEFAULT_SHADER_POOL_SIZE));
	...
	SOKOL_ASSERT(
	    (desc->pipeline_pool_size >= 0) && (desc->pipeline_pool_size < _SG_MAX_POOL_SIZE));
	_sg_init_pool(
	    &p->pipeline_pool, _sg_def(desc->pipeline_pool_size, _SG_DEFAULT_PIPELINE_POOL_SIZE));
	...
	SOKOL_ASSERT(
	    (desc->context_pool_size >= 0) && (desc->context_pool_size < _SG_MAX_POOL_SIZE));
	_sg_init_pool(
	    &p->context_pool, _sg_def(desc->context_pool_size, _SG_DEFAULT_CONTEXT_POOL_SIZE));
...
_SOKOL_PRIVATE const char * _sg_validate_string(_sg_validate_error_t err)
{
	...
	case _SG_VALIDATE_BUFFERDESC_CONTENT:
		return "immutable buffers must be initialized with content "
		       "(sg_buffer_desc.content)";
	...
	case _SG_VALIDATE_IMAGEDESC_NO_MSAA_RT_SUPPORT:
		return "MSAA render targets not supported (SG_FEATURE_MSAA_RENDER_TARGETS)";
	...
	case _SG_VALIDATE_PIPELINEDESC_ATTR_NAME:
		return "GLES2/WebGL vertex layouts must have attribute names";
	case _SG_VALIDATE_PIPELINEDESC_ATTR_SEMANTICS:
		return "D3D11 vertex layouts must have attribute semantics (sem_name and "
		       "sem_index)";
	...
	case _SG_VALIDATE_APIP_ATT_COUNT:
		return "sg_apply_pipeline: color_attachment_count in pipeline doesn't match number "
		       "of pass color attachments";
	case _SG_VALIDATE_APIP_COLOR_FORMAT:
		return "sg_apply_pipeline: color_format in pipeline doesn't match pass color "
		       "attachment pixel format";
	case _SG_VALIDATE_APIP_DEPTH_FORMAT:
		return "sg_apply_pipeline: depth_format in pipeline doesn't match pass depth "
		       "attachment pixel format";
	...
	case _SG_VALIDATE_ABND_VBS:
		return "sg_apply_bindings: number of vertex buffers doesn't match number of "
		       "pipeline vertex layouts";
	...
	case _SG_VALIDATE_ABND_NO_IB:
		return "sg_apply_bindings: pipeline object defines indexed rendering, but no index "
		       "buffer provided";
	case _SG_VALIDATE_ABND_IB:
		return "sg_apply_bindings: pipeline object defines non-indexed rendering, but "
		       "index buffer provided";
	case _SG_VALIDATE_ABND_IB_TYPE:
		return "sg_apply_bindings: buffer in index buffer slot is not a "
		       "SG_BUFFERTYPE_INDEXBUFFER";
	...
	case _SG_VALIDATE_ABND_FS_IMGS:
		return "sg_apply_bindings: fragment shader image count doesn't match "
		       "sg_shader_desc";
	...
	case _SG_VALIDATE_AUB_NO_UB_AT_SLOT:
		return "sg_apply_uniforms: no uniform block declaration at this shader stage UB "
		       "slot";
	...
	case _SG_VALIDATE_UPDATEBUF_APPEND:
		return "sg_update_buffer: cannot call sg_update_buffer and sg_append_buffer in "
		       "same frame";
	...
	case _SG_VALIDATE_APPENDBUF_UPDATE:
		return "sg_append_buffer: cannot call sg_append_buffer and sg_update_buffer in "
		       "same frame";
...
_SOKOL_PRIVATE bool _sg_validate_image_desc(const sg_image_desc * desc)
{
	...
	const bool            ext   = (0 != desc->gl_textures[0]) || (0 != desc->mtl_textures[0]) ||
	                 (0 != desc->d3d11_texture);
	if (desc->render_target) {
		if (desc->sample_count > 1) {
			SOKOL_VALIDATE(
			    _sg_query_feature(SG_FEATURE_MSAA_RENDER_TARGETS),
			    _SG_VALIDATE_IMAGEDESC_NO_MSAA_RT_SUPPORT);
		}
		...
		SOKOL_VALIDATE(
		    valid_color_fmt || valid_depth_fmt, _SG_VALIDATE_IMAGEDESC_RT_PIXELFORMAT);
		SOKOL_VALIDATE(usage == SG_USAGE_IMMUTABLE, _SG_VALIDATE_IMAGEDESC_RT_IMMUTABLE);
		SOKOL_VALIDATE(
		    desc->content.subimage[0][0].ptr == 0, _SG_VALIDATE_IMAGEDESC_RT_NO_CONTENT);
	} else {
		...
		/* FIXME: should use the same "expected size" computation as in
		 * _sg_validate_update_image() here */
		if (!ext && (usage == SG_USAGE_IMMUTABLE)) {
			const int num_faces =
			    _sg_def(desc->type, SG_IMAGETYPE_2D) == SG_IMAGETYPE_CUBE ? 6 : 1;
			const int num_mips = _sg_def(desc->num_mipmaps, 1);
			for (int face_index = 0; face_index < num_faces; face_index++) {
				for (int mip_index = 0; mip_index < num_mips; mip_index++) {
					const bool has_data =
					    desc->content.subimage[face_index][mip_index].ptr != 0;
					const bool has_size =
					    desc->content.subimage[face_index][mip_index].size > 0;
					SOKOL_VALIDATE(
					    has_data && has_size, _SG_VALIDATE_IMAGEDESC_CONTENT);
				}
			}
		} else {
			for (int face_index = 0; face_index < SG_CUBEFACE_NUM; face_index++) {
				for (int mip_index = 0; mip_index < SG_MAX_MIPMAPS; mip_index++) {
					const bool no_data =
					    0 == desc->content.subimage[face_index][mip_index].ptr;
					const bool no_size =
					    0 == desc->content.subimage[face_index][mip_index].size;
					SOKOL_VALIDATE(
					    no_data && no_size, _SG_VALIDATE_IMAGEDESC_NO_CONTENT);
				}
			}
		}
	}
...
_SOKOL_PRIVATE bool _sg_validate_shader_desc(const sg_shader_desc * desc)
{
	...
	SOKOL_VALIDATE(
	    (0 != desc->vs.source) || (0 != desc->vs.byte_code),
	    _SG_VALIDATE_SHADERDESC_SOURCE_OR_BYTECODE);
	SOKOL_VALIDATE(
	    (0 != desc->fs.source) || (0 != desc->fs.byte_code),
	    _SG_VALIDATE_SHADERDESC_SOURCE_OR_BYTECODE);
#elif defined(SOKOL_D3D11)
	...
	if (0 != desc->vs.byte_code) {
		SOKOL_VALIDATE(
		    desc->vs.byte_code_size > 0, _SG_VALIDATE_SHADERDESC_NO_BYTECODE_SIZE);
	}
	if (0 != desc->fs.byte_code) {
		SOKOL_VALIDATE(
		    desc->fs.byte_code_size > 0, _SG_VALIDATE_SHADERDESC_NO_BYTECODE_SIZE);
	}
	for (int stage_index = 0; stage_index < SG_NUM_SHADER_STAGES; stage_index++) {
		const sg_shader_stage_desc * stage_desc =
		...
		for (int ub_index = 0; ub_index < SG_MAX_SHADERSTAGE_UBS; ub_index++) {
			...
			if (ub_desc->size > 0) {
				SOKOL_VALIDATE(
				    uniform_blocks_continuous, _SG_VALIDATE_SHADERDESC_NO_CONT_UBS);
				bool uniforms_continuous = true;
				int  uniform_offset      = 0;
				int  num_uniforms        = 0;
				for (int u_index = 0; u_index < SG_MAX_UB_MEMBERS; u_index++) {
					const sg_shader_uniform_desc * u_desc =
					...
					if (u_desc->type != SG_UNIFORMTYPE_INVALID) {
						...
						const int array_count =
						    _sg_def(u_desc->array_count, 1);
						uniform_offset +=
						    _sg_uniform_size(u_desc->type, array_count);
						num_uniforms++;
					} else {
...
#if defined(SOKOL_GLCORE33) || defined(SOKOL_GLES2) || defined(SOKOL_GLES3)
				SOKOL_VALIDATE(
				    uniform_offset == ub_desc->size,
				    _SG_VALIDATE_SHADERDESC_UB_SIZE_MISMATCH);
				SOKOL_VALIDATE(
				    num_uniforms > 0, _SG_VALIDATE_SHADERDESC_NO_UB_MEMBERS);
#endif
			} else {
		...
		for (int img_index = 0; img_index < SG_MAX_SHADERSTAGE_IMAGES; img_index++) {
			...
			if (img_desc->type != _SG_IMAGETYPE_DEFAULT) {
				SOKOL_VALIDATE(
				    images_continuous, _SG_VALIDATE_SHADERDESC_NO_CONT_IMGS);
#if defined(SOKOL_GLES2)
...
_SOKOL_PRIVATE bool _sg_validate_pipeline_desc(const sg_pipeline_desc * desc)
{
	...
	SOKOL_VALIDATE(
	    shd && shd->slot.state == SG_RESOURCESTATE_VALID, _SG_VALIDATE_PIPELINEDESC_SHADER);
	...
	SOKOL_VALIDATE(
	    desc->layout.attrs[0].format != SG_VERTEXFORMAT_INVALID,
	    _SG_VALIDATE_PIPELINEDESC_NO_ATTRS);
...
_SOKOL_PRIVATE bool _sg_validate_pass_desc(const sg_pass_desc * desc)
{
	...
	for (int att_index = 0; att_index < SG_MAX_COLOR_ATTACHMENTS; att_index++) {
		...
		SOKOL_VALIDATE(
		    img && img->slot.state == SG_RESOURCESTATE_VALID, _SG_VALIDATE_PASSDESC_IMAGE);
		...
		SOKOL_VALIDATE(
		    _sg_is_valid_rendertarget_color_format(img->pixel_format),
		    _SG_VALIDATE_PASSDESC_COLOR_INV_PIXELFORMAT);
	}
	if (desc->depth_stencil_attachment.image.id != SG_INVALID_ID) {
		...
		SOKOL_VALIDATE(img->render_target, _SG_VALIDATE_PASSDESC_IMAGE_NO_RT);
		SOKOL_VALIDATE(
		    width == img->width >> att->mip_level, _SG_VALIDATE_PASSDESC_IMAGE_SIZES);
		SOKOL_VALIDATE(
		    height == img->height >> att->mip_level, _SG_VALIDATE_PASSDESC_IMAGE_SIZES);
		SOKOL_VALIDATE(
		    sample_count == img->sample_count, _SG_VALIDATE_PASSDESC_IMAGE_SAMPLE_COUNTS);
		SOKOL_VALIDATE(
...
_SOKOL_PRIVATE bool _sg_validate_begin_pass(_sg_pass_t * pass)
{
	...
	for (int i = 0; i < SG_MAX_COLOR_ATTACHMENTS; i++) {
		...
		if (att->image) {
			SOKOL_VALIDATE(
			    att->image->slot.state == SG_RESOURCESTATE_VALID,
			    _SG_VALIDATE_BEGINPASS_IMAGE);
			SOKOL_VALIDATE(
			    att->image->slot.id == att->image_id.id, _SG_VALIDATE_BEGINPASS_IMAGE);
		}
	}
	if (pass->ds_att.image) {
		const _sg_attachment_t * att = &pass->ds_att;
		SOKOL_VALIDATE(
		    att->image->slot.state == SG_RESOURCESTATE_VALID, _SG_VALIDATE_BEGINPASS_IMAGE);
		SOKOL_VALIDATE(
		    att->image->slot.id == att->image_id.id, _SG_VALIDATE_BEGINPASS_IMAGE);
	}
	return SOKOL_VALIDATE_END();
...
_SOKOL_PRIVATE bool _sg_validate_apply_pipeline(sg_pipeline pip_id)
{
	...
	SOKOL_VALIDATE(
	    pip->shader->slot.state == SG_RESOURCESTATE_VALID, _SG_VALIDATE_APIP_SHADER_VALID);
	...
	if (pass) {
		/* an offscreen pass */
		SOKOL_VALIDATE(
		    pip->color_attachment_count == pass->num_color_atts,
		    _SG_VALIDATE_APIP_ATT_COUNT);
		SOKOL_VALIDATE(
		    pip->color_format == pass->color_atts[0].image->pixel_format,
		    _SG_VALIDATE_APIP_COLOR_FORMAT);
		SOKOL_VALIDATE(
		    pip->sample_count == pass->color_atts[0].image->sample_count,
		    _SG_VALIDATE_APIP_SAMPLE_COUNT);
		if (pass->ds_att.image) {
			SOKOL_VALIDATE(
			    pip->depth_format == pass->ds_att.image->pixel_format,
			    _SG_VALIDATE_APIP_DEPTH_FORMAT);
		} else {
			SOKOL_VALIDATE(
		...
		SOKOL_VALIDATE(
		    pip->color_format == SG_PIXELFORMAT_RGBA8, _SG_VALIDATE_APIP_COLOR_FORMAT);
		SOKOL_VALIDATE(
		    pip->depth_format == SG_PIXELFORMAT_DEPTHSTENCIL,
		    _SG_VALIDATE_APIP_DEPTH_FORMAT);
...
_SOKOL_PRIVATE bool _sg_validate_apply_bindings(const sg_bindings * bind)
{
	...
	for (int i = 0; i < SG_MAX_SHADERSTAGE_BUFFERS; i++) {
		if (bind->vertex_buffers[i].id != SG_INVALID_ID) {
			 ...
			 */
			const _sg_buffer_t * buf =
			    _sg_lookup_buffer(&_sg.pools, bind->vertex_buffers[i].id);
			SOKOL_ASSERT(buf);
			if (buf->slot.state == SG_RESOURCESTATE_VALID) {
				SOKOL_VALIDATE(
				    SG_BUFFERTYPE_VERTEXBUFFER == buf->type,
				    _SG_VALIDATE_ABND_VB_TYPE);
				SOKOL_VALIDATE(
				    !buf->append_overflow, _SG_VALIDATE_ABND_VB_OVERFLOW);
			}
		} else {
	...
	if (bind->index_buffer.id != SG_INVALID_ID) {
		...
		if (buf->slot.state == SG_RESOURCESTATE_VALID) {
			SOKOL_VALIDATE(
			    SG_BUFFERTYPE_INDEXBUFFER == buf->type, _SG_VALIDATE_ABND_IB_TYPE);
	...
	for (int i = 0; i < SG_MAX_SHADERSTAGE_IMAGES; i++) {
		...
		if (bind->vs_images[i].id != SG_INVALID_ID) {
			...
			const _sg_image_t * img =
			    _sg_lookup_image(&_sg.pools, bind->vs_images[i].id);
			SOKOL_ASSERT(img);
			if (img->slot.state == SG_RESOURCESTATE_VALID) {
				SOKOL_VALIDATE(
				    img->type == stage->images[i].type,
				    _SG_VALIDATE_ABND_VS_IMG_TYPES);
			}
		} else {
			SOKOL_VALIDATE(i >= stage->num_images, _SG_VALIDATE_ABND_VS_IMGS);
	...
	for (int i = 0; i < SG_MAX_SHADERSTAGE_IMAGES; i++) {
		...
		if (bind->fs_images[i].id != SG_INVALID_ID) {
			...
			const _sg_image_t * img =
			    _sg_lookup_image(&_sg.pools, bind->fs_images[i].id);
			SOKOL_ASSERT(img);
			if (img->slot.state == SG_RESOURCESTATE_VALID) {
				SOKOL_VALIDATE(
				    img->type == stage->images[i].type,
...
_SOKOL_PRIVATE bool _sg_validate_apply_uniforms(
...
_SOKOL_PRIVATE bool
_sg_validate_update_buffer(const _sg_buffer_t * buf, const void * data, int size)
{
#if !defined(SOKOL_DEBUG)
	_SOKOL_UNUSED(buf);
	_SOKOL_UNUSED(data);
...
_SOKOL_PRIVATE bool
_sg_validate_append_buffer(const _sg_buffer_t * buf, const void * data, int size)
{
#if !defined(SOKOL_DEBUG)
	_SOKOL_UNUSED(buf);
	_SOKOL_UNUSED(data);
...
_SOKOL_PRIVATE bool
_sg_validate_update_image(const _sg_image_t * img, const sg_image_content * data)
{
#if !defined(SOKOL_DEBUG)
	_SOKOL_UNUSED(img);
	_SOKOL_UNUSED(data);
	...
	SOKOL_VALIDATE(
	    !_sg_is_compressed_pixel_format(img->pixel_format), _SG_VALIDATE_UPDIMG_COMPRESSED);
	const int num_faces = (img->type == SG_IMAGETYPE_CUBE) ? 6 : 1;
	const int num_mips  = img->num_mipmaps;
	for (int face_index = 0; face_index < num_faces; face_index++) {
		for (int mip_index = 0; mip_index < num_mips; mip_index++) {
			SOKOL_VALIDATE(
			    0 != data->subimage[face_index][mip_index].ptr,
			    _SG_VALIDATE_UPDIMG_NOTENOUGHDATA);
			const int mip_width  = _sg_max(img->width >> mip_index, 1);
			const int mip_height = _sg_max(img->height >> mip_index, 1);
			...
			SOKOL_VALIDATE(
			    data->subimage[face_index][mip_index].size <= expected_size,
			    _SG_VALIDATE_UPDIMG_SIZE);
		}
	}
	return SOKOL_VALIDATE_END();
...
SOKOL_API_IMPL sg_context sg_setup_context(void)
{
	...
	if (_SG_INVALID_SLOT_INDEX != slot_index) {
		res.id = _sg_slot_alloc(
		    &_sg.pools.context_pool, &_sg.pools.contexts[slot_index].slot, slot_index);
...
SOKOL_API_IMPL sg_buffer sg_alloc_buffer(void)
{
	...
	if (_SG_INVALID_SLOT_INDEX != slot_index) {
		res.id = _sg_slot_alloc(
		    &_sg.pools.buffer_pool, &_sg.pools.buffers[slot_index].slot, slot_index);
	} else {
		/* pool is exhausted */
		res.id = SG_INVALID_ID;
	}
...
SOKOL_API_IMPL sg_image sg_alloc_image(void)
{
	...
	if (_SG_INVALID_SLOT_INDEX != slot_index) {
		res.id = _sg_slot_alloc(
		    &_sg.pools.image_pool, &_sg.pools.images[slot_index].slot, slot_index);
	} else {
		/* pool is exhausted */
		res.id = SG_INVALID_ID;
	}
...
SOKOL_API_IMPL sg_shader sg_alloc_shader(void)
{
	...
	if (_SG_INVALID_SLOT_INDEX != slot_index) {
		res.id = _sg_slot_alloc(
		    &_sg.pools.shader_pool, &_sg.pools.shaders[slot_index].slot, slot_index);
	} else {
		/* pool is exhausted */
		res.id = SG_INVALID_ID;
	}
...
SOKOL_API_IMPL sg_pipeline sg_alloc_pipeline(void)
{
	...
	if (_SG_INVALID_SLOT_INDEX != slot_index) {
		res.id = _sg_slot_alloc(
		    &_sg.pools.pipeline_pool, &_sg.pools.pipelines[slot_index].slot, slot_index);
	} else {
		/* pool is exhausted */
		res.id = SG_INVALID_ID;
	}
...
SOKOL_API_IMPL sg_pass sg_alloc_pass(void)
{
	...
	if (_SG_INVALID_SLOT_INDEX != slot_index) {
		res.id = _sg_slot_alloc(
		    &_sg.pools.pass_pool, &_sg.pools.passes[slot_index].slot, slot_index);
	} else {
		/* pool is exhausted */
		res.id = SG_INVALID_ID;
	}
	...
	if (_sg_validate_pass_desc(desc)) {
		...
		for (int i = 0; i < SG_MAX_COLOR_ATTACHMENTS; i++) {
			if (desc->color_attachments[i].image.id) {
				att_imgs[i] = _sg_lookup_image(
				    &_sg.pools, desc->color_attachments[i].image.id);
				SOKOL_ASSERT(
				    att_imgs[i] &&
				    att_imgs[i]->slot.state == SG_RESOURCESTATE_VALID);
			} else {
				att_imgs[i] = 0;
			}
		}
...
SOKOL_API_IMPL void sg_destroy_buffer(sg_buffer buf_id)
{
	...
	if (buf) {
		if (buf->slot.ctx_id == _sg.active_context.id) {
			...
			SOKOL_LOG("sg_destroy_buffer: active context mismatch (must be same as for "
			          "creation)");
		}
	}
}
...
SOKOL_API_IMPL void sg_destroy_image(sg_image img_id)
{
	...
	if (img) {
		if (img->slot.ctx_id == _sg.active_context.id) {
			...
			SOKOL_LOG("sg_destroy_image: active context mismatch (must be same as for "
			          "creation)");
		}
	}
}
...
SOKOL_API_IMPL void sg_destroy_shader(sg_shader shd_id)
{
	...
	if (shd) {
		if (shd->slot.ctx_id == _sg.active_context.id) {
			...
			SOKOL_LOG("sg_destroy_shader: active context mismatch (must be same as for "
			          "creation)");
		}
	}
}
...
SOKOL_API_IMPL void sg_destroy_pipeline(sg_pipeline pip_id)
{
	...
	if (pip) {
		if (pip->slot.ctx_id == _sg.active_context.id) {
			...
			SOKOL_LOG("sg_destroy_pipeline: active context mismatch (must be same as "
			          "for creation)");
		}
	}
}
...
SOKOL_API_IMPL void sg_destroy_pass(sg_pass pass_id)
{
	...
	if (pass) {
		if (pass->slot.ctx_id == _sg.active_context.id) {
			...
			SOKOL_LOG("sg_destroy_pass: active context mismatch (must be same as for "
			          "creation)");
		}
	}
}
...
SOKOL_API_IMPL void sg_apply_bindings(const sg_bindings * bind)
{
	...
	if (_sg.next_draw_valid) {
		...
		_sg_apply_bindings(
		    pip, vbs, vb_offsets, num_vbs, ib, ib_offset, vs_imgs, num_vs_imgs, fs_imgs,
		    num_fs_imgs);
	}
}
		...
		if (buf->slot.state == SG_RESOURCESTATE_VALID) {
			if (_sg_validate_append_buffer(buf, data, num_bytes)) {
				if (!buf->append_overflow && (num_bytes > 0)) {
					/* update and append on same buffer in same frame not
					 * allowed */
					SOKOL_ASSERT(buf->update_frame_index != _sg.frame_index);
					_sg_append_buffer(
					    buf, data, num_bytes,
